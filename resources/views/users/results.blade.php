<x-default :heading="$heading">
    @php
        ...
        $completed = $elections->where('status', 'Completed')->where('for', $forWhat)->where('campus', $campusDictionary[$idAlpha])->get();
    @endphp
    ...
    class = "bg-darkGreenBPC w-full h-[300px] bg-cover bg-center bg-no-repeat relative pt-[200px]">
        ...
        <div class = "w-full z-20 text-white flex justify-center relative">
            <h2 class = "text-white text-3xl font-medium">Election Results</h2>
        </div>  
    </section>
    ...
    <section class="mt-[20px]">
        ...
        <div class = "w-full bg-gray-100 py-3 mt-[-20px] mb-3 flex justify-center sm:justify-start">
            <form class = "mx-8 flex relative rounded-lg" method="GET" action="{{URL('/users/searchResults')}}">
                @csrf
        ...
        @if (count($completed) != 0)
            <div class = "w-full px-5 flex flex-col md:flex-row flex-wrap">
                ...
                @foreach ($completed as $election)
                    <a href = '{{URL('electionResult/' . $election->id)}}' class = "bg-white w-full p-4 electionCard rounded-xl shadow-md my-3 border border-gray-100 md:w-[calc(50%-16px)] md:mx-2" type="button" data-modal-toggle="default-modal">
