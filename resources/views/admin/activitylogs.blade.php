            ...
            <div class="md:col-span-2 lg:col-span-1" >
                <div class="h-full py-8 px-6 space-y-6 rounded-xl border rounded-tl-none overflow-auto border-gray-200 bg-white min-h-[calc(100vh-120px)]">
                    <div class = "flex w-full justify-between min-w-[900px]">
                        ...
                        <form class = "flex relative rounded-lg gap-1" action="{{URL('admin/logs')}}" method="GET">
                            @csrf
                    ...
                    <table class="min-w-[900px] text-sm md:text-base overflow-auto w-full " id="main-table">
                        <thead class = "" id = "thead">
                            <th class = "max-w-[200px]">Activity ID</th>
                            <th class = "max-w-[200px]">Admin</th>
                            <th class = "max-w-[200px]">Activity</th>
                            <th class = "max-w-[200px]">Date</th>
                        </thead>
                        @if (count($logs) == 0)
                        ...
                        @else
                            @foreach($logs as $log)
                            <tr class = "text-center max-h-[80vh]">
                                <td class = "max-w-[200px]">{{$log->id}}</td>
                                <td class = "max-w-[200px]">{{$log->admin}}</td>
                                <td class = "max-w-[200px]">{{$log->activity}}</td>
                                <td class = "max-w-[200px]">{{$log->created_at}}</td>
                            </tr>
                            @endforeach
                        @endif
                    </table>
